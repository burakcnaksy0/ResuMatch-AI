...
export class AuthService {
    ...
    async register(registerDto: RegisterDto) {
        ...
        this.mailService.sendWelcomeEmail(email, email.split('@')[0]).catch(err => {
    ...
    async resendVerificationCode(userId: string) {
        ...
        try {
            ...
            if (isDevelopment) {
                this.logger.warn(`DEV MODE: Verification code for ${user.email}: ${verificationCode}`);
                return {
    ...
    async changePassword(userId: string, changePasswordDto: any) {
        ...
        if (!passwordRegex.test(newPassword)) {
            throw new BadRequestException('Password must be at least 8 characters long and contain at least one uppercase letter and one number');
        }
        ...
        this.mailService.sendPasswordChangeNotification(user.email).catch(e => this.logger.error(e));
    ...
    async googleLogin(token: string) {
        try {
            ...
            if (clientId && this.googleClient) {
                ...
                this.logger.warn('GOOGLE_CLIENT_ID not configured. Attempting unsafe decode for development.');
