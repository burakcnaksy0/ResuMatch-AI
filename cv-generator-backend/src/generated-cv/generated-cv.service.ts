...
export class GeneratedCvService {
  ...
  async generateCV(
    ...
    const canGenerate = await this.subscriptionService.canGenerateCV(
      userId,
      cvType,
    );
    if (!canGenerate) {
  ...
  async generatePdf(id: string) {
    ...
    const templateData = {
      ...
      profilePictureUrl:
        cv.cvSpecificPhotoUrl ||
        (cv.includeProfilePicture
          ? cv.profile?.profilePictureUrl || undefined
          : undefined),
    };
    ...
    relativePath = await this.pdfService.generatePdf(
      templateData,
      outputFilename,
      templateName,
    );
